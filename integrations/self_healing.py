...
def cacar_elemento_universal(
    ...
    # Desliga a espera implícita do Selenium temporariamente. 
    # Se não fizermos isso, cada seletor falho custa 5 a 15 segundos!
    try:
...
def _enviar_mensagem_gemini_acessibilidade(driver_acessibilidade, url_gemini, screenshot_path: Path, prompt_texto: str, timeout: int = 60) -> str | None:
    ...
    """
    try:
        # 🛡️ GUARD: Verifica se o driver do Médico ainda está vivo antes de qualquer coisa
        try:
            _ = driver_acessibilidade.title  # Operação mínima — falha instantaneamente se morto
        except Exception as e_vida:
            erro = str(e_vida).lower()
            if any(d in erro for d in ['max retries', 'connectionrefused', 'newconnectionerror',
                ...
                _log(f"🏥 Médico MORTO detectado ({str(e_vida)[:50]}). Tentando ressuscitar...")
                try:
                    from config import Settings
                    from integrations.google_login import garantir_medico_vivo
                    settings = Settings()
                    driver_acessibilidade = garantir_medico_vivo(driver_acessibilidade, settings, url_gemini)
                    _log("🏥 ✅ Médico ressuscitado com sucesso!")
                except Exception as e_revive:
                    _log(f"🏥 ❌ Falha ao ressuscitar Médico: {str(e_revive)[:60]}")
                    return None
            else:
                _log(f"🏥 ⚠️ Médico com erro inesperado: {str(e_vida)[:60]}")
                return None
