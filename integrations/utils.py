...
# ===========================================
# 🧵 THREAD-LOCAL: Contexto automático de thread para logs
# ===========================================
import threading as _threading
_thread_context = _threading.local()

def set_thread_id(tid: int) -> None:
    """Define o thread_id para a thread atual. Todas as chamadas _log subsequentes usarão este ID."""
    _thread_context.thread_id = tid

def _log(msg: str, prefixo: str = "SISTEMA", thread_id: int | None = None) -> None:
    """
    Logger centralizado com suporte a threads.
    
    Console: [15:30:01] [T2] [FLOW] Gerando cena...     (com thread_id)
    Console: [15:30:01] [SISTEMA] Iniciando              (sem thread_id)
    TXT Geral: log_execucao.txt                          (TODAS as mensagens)
    TXT Thread: log_execucao_thread2.txt                 (só mensagens da thread 2)
    """
    import threading as _threading
    from datetime import datetime
    agora = datetime.now()
    ...
    # Identifica a thread (auto-detecta se não foi passado)
    _tid = thread_id
    if _tid is None:
        # 🛡️ Primeiro tenta o thread-local (setado pelo worker ao iniciar)
        _tid = getattr(_thread_context, 'thread_id', None)
    if _tid is None:
        # Fallback: tenta extrair do nome da thread (ThreadPoolExecutor)
        t_name = _threading.current_thread().name
        if t_name != "MainThread" and "Thread" in t_name:
            _tid = -1  # Thread secundária sem ID explícito
    
    # Formata com identificação de thread
    ...
    texto_console = f'[{ts_console}] {tag_thread}[{prefixo}] {msg}'
    texto_file = f'[{ts_file}] {tag_thread}[{prefixo}] {msg}'
    
    try:
        print(texto_console)
    except UnicodeEncodeError:
        # Fallback: remove caracteres que o console nao suporta
        print(texto_console.encode('utf-8', errors='replace').decode('utf-8', errors='replace'))
    
    # Salva no log geral (TODAS as threads)
    try:
    ...
    # Salva no log dedicado da thread (se thread_id informado)
    if _tid is not None and _tid >= 0:
        try:
            log_thread_file = Path("logs") / f"log_execucao_thread{_tid}.txt"
            with open(log_thread_file, "a", encoding="utf-8") as f:
                f.write(texto_file + "\n")
        except: pass

def log_step(message: str, thread_id: int | None = None) -> None:
    _log(message, prefixo="SISTEMA", thread_id=thread_id)

def log_success(message: str, thread_id: int | None = None) -> None:
    _log(f'OK: {message}', prefixo="SISTEMA", thread_id=thread_id)

def log_error(message: str, thread_id: int | None = None) -> None:
    _log(f'ERRO: {message}', prefixo="SISTEMA", thread_id=thread_id)
    
def salvar_print_debug(driver, nome_fase: str, thread_id: int | None = None):
    """Sua função mestre com a tarja vermelha de URL.
    Screenshots são separados por thread em logs/visao/thread_N/ quando thread_id é fornecido.
    """
    load_dotenv(override=True)
    if os.getenv("DISABLE_SCREENSHOTS", "False").lower() == "true":
        return
    try:
        # Subpasta por thread para diagnóstico isolado (padrão thread_0 para root)
        # 🧵 Auto-detecta thread_id via thread-local se não foi passado
        if thread_id is None or thread_id < 0:
            thread_id = getattr(_thread_context, 'thread_id', 0)
            
        pasta_visao = _get_logs_dir() / "visao" / f"thread_{thread_id}"
        ...
        # Prefixo com thread_id para facilitar diagnóstico
        tag_thread = f"T{thread_id}_" if thread_id is not None and thread_id >= 0 else ""
        timestamp = time.strftime('%H%M%S')
        driver.save_screenshot(str(pasta_visao / f"{tag_thread}{timestamp}_{nome_fase}.png"))
...
def limpar_diretorio_visao() -> None:
    """Zera o diretório de prints de monitoramento (logs/visao + subpastas de thread)."""
    try:
        ...
        if pasta_visao.exists():
            ...
            # rglob limpa tanto a raiz quanto subpastas thread_N/
            for arquivo in pasta_visao.rglob("*.png"):
                try:
                    arquivo.unlink()
                except Exception:
                    pass
...
def obter_proxy_por_thread(thread_id: int) -> str | None:
    """
    Retorna um proxy DETERMINÍSTICO para a thread, garantindo que cada
    thread paralela use um IP diferente (round-robin sobre proxies.txt).
    """
    caminho = Path("proxies.txt")
    ...
    escolhido = proxies[thread_id % len(proxies)]
    return escolhido

def obter_proxy_por_conta(email: str) -> str | None:
    """Proxy FIXO por conta (determinístico via hash do email).
    
    Garante que a mesma conta Google sempre use o mesmo IP,
    independente da thread ou retry. Isso evita que o Google
    detecte a mesma conta aparecendo de IPs diferentes.
    """
    caminho = Path("proxies.txt")
    ...
    # Hash determinístico: mesma conta = mesmo proxy SEMPRE
    idx = hash(email.strip().lower()) % len(proxies)
    escolhido = proxies[idx]
    return escolhido
...
def extrair_e_salvar_legenda(texto_roteiro: str, caminho_arquivo: Path, num_roteiro: int) -> bool:
    try:
        ...
        # Permissivo: aceita com/sem dois pontos, com/sem quebra de linha logo depois
        match = re.search(
            r'\[Ll?egenda\]:?\s*(.+?)(?=\n===|\n---|\Z)',
            texto_roteiro,
        ...
        if not legenda_completa:
            # Pega o texto do VOICEOVER mesmo que a IA não coloque aspas
            matches = re.findall(r'VOICEOVER:\s*"?([^"\n]+)"?', texto_roteiro, re.IGNORECASE)
            if matches:
...
def limpar_logs_antigos(horas: int = 12) -> None:
    """Lê os arquivos de log e mantém apenas as linhas das últimas X horas.
    Processa o log geral (log_execucao.txt) e todos os logs de thread (log_execucao_threadN.txt).
    """
    try:
        logs_dir = Path("logs")
        ...
        # Coleta todos os arquivos de log: geral + threads
        arquivos_log = list(logs_dir.glob("log_execucao*.txt"))
            ...
            try:
                ...
                if len(linhas_mantidas) < len(linhas):
                    if linhas_mantidas:
                        ...
                        # Log inteiro está expirado — remove o arquivo
...
_ARQUIVO_PIDS = Path(__file__).parent.parent / "meus_pids.txt"

def registrar_pid_processo(pid: int):
    """Guarda o ID do Chrome para extermínio futuro."""
    try:
        with open(_ARQUIVO_PIDS, "a") as f:
            f.write(f"{pid}\n")
    except: pass
...
def limpar_meus_zumbis():
    ...
    import subprocess
    if not _ARQUIVO_PIDS.exists():
        return
    mortos = 0
    with open(_ARQUIVO_PIDS, "r") as f:
    ...
    _ARQUIVO_PIDS.unlink(missing_ok=True)
    if mortos > 0:
        _log(f"🧹 {mortos} processo(s) do script eliminados via meus_pids.txt.", "LIMPEZA")

def limpar_chromes_orfaos():
    """
    Mata processos Chrome/chromedriver de sessoes anteriores do script.
    Identifica pelo command line (--noerrdialogs, user-data-dir=Temp, perfis_selenium).
    NAO toca nos Chrome pessoais do usuario.
    """
    import subprocess
    marcas_script = ['noerrdialogs', 'perfis_selenium', 'perfil_medico', 'perfis\\\\medico']
    mortos = 0
    try:
        # Busca todos os processos chrome e chromedriver
        result = subprocess.run(
            ['wmic', 'process', 'where', 
             "name like '%chrome%' or name like '%chromedriver%'",
        ...
        for line in result.stdout.splitlines():
            ...
            if not pid.isdigit():
                continue
            # So mata se tem marca do script
            if any(marca in cmd for marca in marcas_script):
                try:
                    subprocess.run(['taskkill', '/f', '/pid', pid, '/T'],
                                   capture_output=True, timeout=5)
                    mortos += 1
        ...
        pass
    if mortos > 0:
        _log(f"🧹 {mortos} processo(s) Chrome orfao(s) de sessoes anteriores eliminados.", "LIMPEZA")
