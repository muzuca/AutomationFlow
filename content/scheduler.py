...
# Horários alvo de publicação (horas inteiras) — configurável via .env
_raw = os.getenv("HORARIOS_PUBLICACAO", "9,12,17,19,21")
HORARIOS_PUBLICACAO = [int(h.strip()) for h in _raw.split(",") if h.strip().isdigit()]

# Janelas de geração: começa X horas antes de cada horário alvo
ANTECEDENCIA_HORAS = float(os.getenv("ANTECEDENCIA_HORAS", "1.5"))

# Tolerância em minutos para considerar que está "na janela"
TOLERANCIA_MINUTOS = int(os.getenv("TOLERANCIA_MINUTOS", "30"))
