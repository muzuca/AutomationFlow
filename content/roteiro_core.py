...
def _corrigir_dialogo_cena_isolada(
    ...
    min_palavras: int,
    max_palavras: int,
    fn_gerar_texto: Callable[[str, str], str] | None = None,
) -> str:
    """
    ...
    for tentativa in range(1, tentativas_locais + 1):
        try:
            ...
            if fn_gerar_texto is None:
                raise RuntimeError("fn_gerar_texto não fornecida para correção de diálogo.")
            ultimo_texto_bruto = fn_gerar_texto(mensagem_cena, instrucao_sistema)
...
def gerar_roteiro_generico(
    ...
    for tentativa in range(1, tentativas + 1):
        ...
        try:
            ...
            if fn_gerar_texto is None:
                raise RuntimeError(
                    "fn_gerar_texto não fornecida. "
                    "Passe uma função de geração (Gemini browser ou API)."
                )
            texto_bruto = fn_gerar_texto(mensagem_com_feedback, instrucao_sistema)
            ...
            for i, cena_dados in enumerate(cenas_json):
                ...
                # ── Propagação de campos visuais GLOBAIS ──────────────
                # O Gemini pode gerar background/outfit/lighting/audio
                # no nível raiz (globais para todo o vídeo).
                # Se a cena não tem esses campos, herda do global.
                for campo_global in ("background", "outfit", "lighting", "audio"):
                    if not cena_dados.get(campo_global):
                        valor_global = dados.get(campo_global, "")
                        if valor_global:
                            cena_dados[campo_global] = valor_global
                
                # Injeta metadados de posição para o builder de prompt
                cena_dados["_cena_idx"] = i + 1
                cena_dados["_total_cenas"] = n_cenas

                # Usa cena_dados (resposta do Gemini) que contem campos criativos
                # (background, action, lighting, outfit, audio) em vez de cena_base
                prompt_completo = builder_prompt_veo3(cena_dados, dialogo)
            ...
            return {
                ...
                "cenas": cenas_json,
                "marcadores": dados.get("marcadores", {}),
            }
            ...
            # 🚨 Tab crash = browser MORTO, não adianta re-tentar na mesma sessão
            if "tab crashed" in msg.lower() or "tab crash" in msg.lower():
                raise RuntimeError("SWITCH_ACCOUNT: Tab crashed — browser morto, trocando de conta")
