...
def executar_ciclo(config: dict):
    ...
    for rodada in range(1, qtd + 1):
        ...
        for pcfg in config["personagens"]:
            ...
            print(f"  Motor: {label_motor} | Vídeo desta rodada: {rodada}/{qtd} | Cenas/vídeo: {cenas}")
            linha("-", 55)
            print(f"\n  VÍDEO {rodada}/{qtd} — {nome}")

            roteiro, tema_final, mensagem, signo_final = gerar_roteiro_sem_repeticao(
                pid=pid, signo_config=signo, tema_escolhido=tema, cenas_por_video=cenas,
            )
            ...
            print(f"✔ Roteiro gerado — tema: {tema_final} | signo: {signo_final or '—'}")
            print(f"  [{label_motor}] Gerando {len(roteiro['prompts'])} cenas...")

            # chame igual hoje; o destino final ainda é decidido no video_manager
            if label_motor == "Humble":
            ...
            caminho_final = processar_videos(
                personagem=pid, arquivos=arquivos, tema=tema_final, signo=signo_final,
            )

            if caminho_final:
                registrar_roteiro(pid, signo=signo_final or "—", tema=tema_final, roteiro=roteiro)
                resultados.append({"personagem": nome, "arquivo": caminho_final,
                                   "tema": tema_final, "signo": signo_final or "—"})
...
def main():
    ...
    # Chama a sincronização logo no início
